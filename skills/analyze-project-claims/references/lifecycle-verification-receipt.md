...
In the commands below, replace `<skill-root>` with the path to the installed
`analyze-project-claims` directory that contains `SKILL.md`. This works from a
standalone skill install, a plugin install, or a publisher checkout.

```powershell
py -3 "<skill-root>\scripts\lifecycle_receipt.py" `
